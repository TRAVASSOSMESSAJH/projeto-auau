...
try {
    ...
    echo "Deu merda ae irmão: " . $e->getMessage();
}
...
try{
    function insereNovoCachorro($nome,$senha,$raca,$telefone,$dataDeNascimento, $descricao, $foto, $sexo){
        ...
        echo "Foi ai truta, funcionou meu parceiro, meu irmão, meu querido divirtasse aqui nessa espelunca cheia de malhuco doido hehehe!";
        return true;
    }
    ...
    function salvaAmizade($nome,$senha,$nome2,$senha2){
        ...
        echo "agora só espera o irmão querer ser teu parça mlk";
        return true;
    }
    ...
    function exibePosts(){
        global $conexao;      
        $rs = $conexao->query("SELECT * FROM feed ORDER BY dateTime DESC;");
        while($row = $rs->fetch(PDO::FETCH_OBJ)){  
           $date = date_create($row->dateTime);
           $date = date_format($date, 'd/m/Y H:i:s');
           echo "<div class='flex vertical post'>";
           echo "<div class='flex horizontal'>";
           echo "<b style='padding-right: 10px;'>$row->usernameCreator</b><i style='font-size: smaller;'>$date </i></div>";
           echo "<p>$row->text</p>";
           echo "</div>";
        }
    }
    ...
    function insereNovoPost($username, $post){
        global $conexao;
        $data= 0; 
        $stm = $conexao->prepare("INSERT INTO feed (usernameCreator, text, dateTime) 
        VALUES (:username, :post, 0)");
        
        //$stm->bindParam(':usernameCreator', $username);
        //$stm->bindParam(':text', $post);
        //$stm->bindParam(':dateTime', $data);
        $stm->execute();
    ...
    echo "Deu merda ai irmão: " . $e->getMessage();
